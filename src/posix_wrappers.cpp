...
namespace jsh {
named_pipe_wrapper::named_pipe_wrapper(std::string pipe_name)
    : success{true}, name{std::move(pipe_name)} {
    // create the pipe
    if (mkfifo(name.c_str(), PIPE_PERMS) == -1) {
        std::cout << "Error making pipe: "
                  << syscall_wrapper::strerror_wrapper(errno) << '\n';
        success = false;
    }
}
...
file_descriptor_wrapper::file_descriptor_wrapper(
    file_descriptor_wrapper&& other) noexcept
    : _fides{other._fides} {
    // set the other fdw to invalid
    other._fides = -1;
}
auto file_descriptor_wrapper::operator=(
    file_descriptor_wrapper&& other) noexcept -> file_descriptor_wrapper& {
...
file_descriptor_wrapper::file_descriptor_wrapper(int fides) noexcept
    : _fides{fides} {
...
file_descriptor_wrapper::~file_descriptor_wrapper() {
    ...
    if (_fides != -1) {
        ...
        if (status == -1) {
            cout_logger.log(LOG_LEVEL::ERROR,
...
auto file_descriptor_wrapper::operator!=(
    file_descriptor_wrapper const& other) const -> bool {
...
auto syscall_wrapper::open_wrapper(std::string const& file, int flags,
                                   mode_t perms)
    ...
    // check for errors
    if (fides == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Error occurred while opening file ", file, ": ",
                        strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::dup_wrapper(file_descriptor_wrapper const& fides)
    -> std::optional<file_descriptor_wrapper> {
    ...
    if (new_fides == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Error occurred while duplicating a file descriptor: ",
                        strerror_wrapper(errno));
        return std::nullopt;
    }

    // create the new file descriptor
    return std::make_optional<file_descriptor_wrapper>(
        file_descriptor_wrapper(new_fides));
}

auto syscall_wrapper::dup2_wrapper(file_descriptor_wrapper const& fides1,
                                   file_descriptor_wrapper const& fides2)
    -> bool {
    ...
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Error occurred while duplicating2 a file descriptor: ",
                        strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::pipe_wrapper()
    ...
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Creating a anonymous pipe: ", strerror_wrapper(errno));
        return std::nullopt;
    }
    ...
    return std::make_optional<std::vector<file_descriptor_wrapper>>(
        std::move(fides_wrappers));
}

auto syscall_wrapper::read_wrapper(file_descriptor_wrapper const& fides,
    ...
    if (status == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::ERROR,
            "Error while reading file descriptor: ", strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::isatty_wrapper(file_descriptor_wrapper const& fides)
    -> bool {
    ...
    if (status != 1) {
        cout_logger.log(
            jsh::LOG_LEVEL::WARN,
            "JSH is not running as a terminal: ", strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::tcgetpgrp_wrapper(file_descriptor_wrapper const& fides)
    -> std::optional<pid_t> {
    // run
    ...
    if (tc_grp_pid == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::WARN,
            "Failed to get the process group controlling the terminal: ",
            strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::getpgrp_wrapper() -> std::optional<pid_t> {
    ...
    if (pgrp_id == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::ERROR,
            "Failed to get the process group id: ", strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::setpgid_wrapper(pid_t pid, pid_t pgid) -> bool {
    ...
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Failed to set the process' process group id: ",
                        strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::tcsetpgrp_wrapper(
    file_descriptor_wrapper const& term_fides, pid_t shell_pid) -> bool {
    ...
    if (status == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::ERROR,
            "Failed to set the terminal's controlling process group: ",
            strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::tcgetattr_wrapper(
    file_descriptor_wrapper const& term_fides,
    ...
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Failed to get the terminal's interface attribute: ",
                        strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::tcsetattr_wrapper(
    file_descriptor_wrapper const& term_fides, int opts,
    std::shared_ptr<termios> const& term_if) -> bool {
    // set the interface
    ...
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Failed to set the terminal's interface attribute: ",
                        strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::kill_wrapper(pid_t pid, int sig) -> bool {
    ...
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Failed to send signal: ", strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::signal_wrapper(int sig, void (*func)(int))
    ...
    // error handle
    if (ret == SIG_ERR) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR, "Failed to set signal handler: ",
                        strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::sigemptyset_wrapper(sigset_t& sigset) -> bool {
    ...
    // error handle
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR, "Failed to empty signal set: ",
                        strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::sigaddset_wrapper(sigset_t& sigset, int signo) -> bool {
    ...
    // error handle
    if (status == -1) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR, "Failed to add signal to set: ",
                        strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::sigprocmask_wrapper(int how, sigset_t& set) -> bool {
    ...
    if (status == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::ERROR,
            "Failed to create process signal mask: ", strerror_wrapper(errno));
        return false;
    }
...
auto syscall_wrapper::signalfd_wrapper(file_descriptor_wrapper const& fides,
                                       sigset_t& set, int flags)
    -> std::optional<file_descriptor_wrapper> {
    ...
    // create the new file descriptor
    return std::make_optional<file_descriptor_wrapper>(
        file_descriptor_wrapper(new_signal_fd));
}

auto syscall_wrapper::poll_wrapper(std::vector<pollfd_wrapper>& fds,
    ...
    if (num_fds == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::ERROR,
            "Failed to poll file descriptors: ", strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::fork_wrapper() -> std::optional<pid_t> {
    ...
    if (pid == -1) {
        cout_logger.log(
            jsh::LOG_LEVEL::ERROR,
            "Failed to fork file descriptors: ", strerror_wrapper(errno));
        return std::nullopt;
    }
...
auto syscall_wrapper::strerror_wrapper(int err) -> bool {
    ...
    if (status == nullptr) {
        cout_logger.log(jsh::LOG_LEVEL::ERROR,
                        "Failed convert error to string: ",
                        strerror_r(err, err_buf.data(), err_buf.size()));
        return false;
    }
